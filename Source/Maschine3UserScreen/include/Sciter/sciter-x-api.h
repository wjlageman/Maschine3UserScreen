...
#define SCITER_API_VERSION 10

typedef void(* proc_ptr_t)(void);
...
inline ISciterAPI *_SAPI(ISciterAPI *ext) {
  ...
  inline SBOOL  SCAPI SciterAtomNameCB(UINT64 atomv, LPCSTR_RECEIVER* rcv, LPVOID rcv_param) { return SAPI()->SciterAtomNameCB(atomv, rcv, rcv_param); }
  inline SBOOL  SCAPI SciterSetGlobalAsset(som_asset_t* pass) { return SAPI()->SciterSetGlobalAsset(pass); }
  inline SBOOL  SCAPI SciterReleaseGlobalAsset(som_asset_t* pass) { return SAPI()->SciterReleaseGlobalAsset(pass); }
  ...
  inline proc_ptr_t SCAPI SciterEGLGetProcAddress(char const* procName) {
    return SAPI()->SciterEGLGetProcAddress(procName);
  }

  inline SCDOM_RESULT SCAPI SciterEGLSendEvent(HELEMENT he, UINT eventCode, UINT_PTR reason) {
    return SAPI()->SciterEGLSendEvent(he, eventCode, reason);
  }
