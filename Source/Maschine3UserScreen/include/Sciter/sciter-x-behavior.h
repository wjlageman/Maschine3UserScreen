...
#pragma pack(pop)
  ...
  namespace sciter
  {
    struct event_handler_raw // "raw", non-reference counted event handler
    {
      ...
      virtual void handle_style_change(HELEMENT he, UINT change_kind) { }
      ...
      static SBOOL SC_CALLBACK  element_proc(LPVOID tag, HELEMENT he, UINT evtg, LPVOID prms )
      {
        ...
        if( pThis ) switch( evtg )
        {
            ...
            case HANDLE_STYLE_CHANGE: { UINT change_kind = (UINT)((UINT_PTR)prms); pThis->handle_style_change(he, change_kind); return 0; }
			      default:
              assert(false);
        }
        return false;
