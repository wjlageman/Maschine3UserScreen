...
//        }
      ...
      // STATIC method: sets variable that will be available in each document 
      //                loaded after this call 
      static void set_shared_variable(const char* name, sciter::value val) {
        SciterSetVariable(NULL, name, &val);
      }

      // sets variable that will be available in root document of this window,
      // call it in or after DOCUMENT_CREATED event
      void set_root_document_variable(const char* name, sciter::value val) {
        SciterSetVariable(static_cast<BASE*>(this)->get_hwnd(), name, &val);
      }
